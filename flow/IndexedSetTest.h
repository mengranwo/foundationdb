...
#ifndef FLOW_INDEXEDSETTEST_H
#define FLOW_INDEXEDSETTEST_H
#pragma once
...
public:
	struct iterator{
		typename IndexedSetTest::Node *i;
		iterator() : i(0) {};
		iterator(typename IndexedSetTest::Node *n) : i(n) {};
	...
	IndexedSetTest() : root(NULL) {};
	~IndexedSetTest() { delete root; }
	IndexedSetTest(IndexedSetTest&& r) noexcept(true) : root(r.root) { r.root = NULL; }
	IndexedSetTest& operator=(IndexedSetTest&& r) noexcept(true) { delete root; root = r.root; r.root = 0; return *this; }
	...
	void swap( IndexedSetTest& r ) { std::swap( root, r.root ); }
...
private:
	...
	IndexedSetTest( const IndexedSetTest& );
	IndexedSetTest& operator=( const IndexedSetTest& );
...
template <class T, class Metric>
void IndexedSetTest<T,Metric>::iterator::operator++(){
	moveIterator<1>(i);
}

template <class T, class Metric>
void IndexedSetTest<T,Metric>::iterator::decrementNonEnd(){
	moveIterator<0>(i);
}
...
template <class T, class Metric>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::begin() const {
	Node *x = root;
	while (x && x->child[0])
		x = x->child[0];
	return x;
...
template <class T, class Metric>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::previous(typename IndexedSetTest<T,Metric>::iterator i) const {
	if (i==end())
		return lastItem();
...
template <class T, class Metric>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::lastItem() const {
	Node *x = root;
	while (x && x->child[1])
		x = x->child[1];
	return x;
...
Metric IndexedSetTest<T,Metric>::addMetric(T_&& data, Metric_&& metric){
	auto i = find( data );
	if (i == end()) {
...
template <class T, class Metric> template<class T_, class Metric_>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::insert(T_&& data, Metric_&& metric, bool replaceExisting){
	if (root == NULL){
...
template <class T, class Metric>
int IndexedSetTest<T,Metric>::insert(const std::vector<std::pair<T,Metric>>& dataVector, bool replaceExisting) {
	int num_inserted = 0;
...
template <class T, class Metric>
Metric IndexedSetTest<T,Metric>::eraseHalf( Node* start, Node* end, int eraseDir, int& heightDelta, std::vector<Node*>& toFree ) {
	...
	while(start != end) {
		...
		if (fromDir==eraseDir) {
			...
			IndexedSetTest<T,Metric>::Node *n = node->child[fromDir];
...
template <class T, class Metric>
void IndexedSetTest<T,Metric>::erase( typename IndexedSetTest<T,Metric>::iterator begin, typename IndexedSetTest<T,Metric>::iterator end, std::vector<Node*>& toFree ) {
	...
	IndexedSetTest<T,Metric>::Node* first = begin.i;
	IndexedSetTest<T,Metric>::Node* last = previous(end).i;

	IndexedSetTest<T,Metric>::Node* subRoot = iSCommonSubtreeRoot(first, last);
	...
	erase( IndexedSetTest<T,Metric>::iterator(subRoot) );
}

template <class T, class Metric>
void IndexedSetTest<T,Metric>::erase(iterator toErase) {
	Node* rebalanceNode;
	int rebalanceDir;
...
template <class T, class Metric>
template <class Key>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::find(const Key &key) const {
	Node* t = root;
	while (t){
		int d = t->data < key;
...
template <class T, class Metric>
template <class Key>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::lower_bound(const Key &key) const {
	Node* t = root;
...
template <class T, class Metric>
template <class Key>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::upper_bound(const Key &key) const {
	Node* t = root;
	if (!t) return iterator();
	while (true) {
...
template <class T, class Metric>
template <class Key>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::lastLessOrEqual(const Key &key) const {
...
template <class T, class Metric>
template <class M>
typename IndexedSetTest<T,Metric>::iterator IndexedSetTest<T,Metric>::index( M const& metric ) const
{
	M m = metric;
	Node* t = root;
	while (t) {
...
template <class T, class Metric>
Metric IndexedSetTest<T,Metric>::getMetric(typename IndexedSetTest<T,Metric>::iterator x) const {
	Metric m = x.i->total;
	for(int i=0; i<2; i++)
		if (x.i->child[i]) 
...
template <class T, class Metric>
Metric IndexedSetTest<T,Metric>::sumTo(typename IndexedSetTest<T,Metric>::iterator end) const {
	if (!end.i)
...
template <class T, class Metric>
void IndexedSetTest<T,Metric>::erase(typename IndexedSetTest<T,Metric>::iterator begin, typename IndexedSetTest<T,Metric>::iterator end) {
	std::vector<IndexedSetTest<T,Metric>::Node*> toFree;
...
template <class T, class Metric>
template <class Key>
Future<Void> IndexedSetTest<T, Metric>::eraseAsync(const Key &begin, const Key &end) {
	return eraseAsync(lower_bound(begin), lower_bound(end) ); 
}

template <class T, class Metric>
Future<Void> IndexedSetTest<T, Metric>::eraseAsync(typename IndexedSetTest<T,Metric>::iterator begin, typename IndexedSetTest<T,Metric>::iterator end) {
	std::vector<IndexedSetTest<T,Metric>::Node*> toFree;
	erase(begin, end, toFree);
