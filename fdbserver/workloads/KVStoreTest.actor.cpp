...
ACTOR Future<Void> testKVStoreMain(KVStoreTestWorkload* workload, KVTest* ptest) {
	...
	if (workload->doCount) {
		...
		while (true) {
			Standalone<RangeResultRef> kv =
			    wait(test.store->readRange(KeyRangeRef(k, LiteralStringRef("\xff\xff\xff\xff")), 50));
			count += kv.size();
			if (kv.size() < 50) break;
	...
	if (workload->doSetup) {
		...
		for (i = 0; i < workload->nodeCount; i++) {
			test.store->set(KeyValueRef(test.makeKey(i), StringRef(std::to_string(i))));
			if (!((i + 1) % 100) || i + 1 == workload->nodeCount) {
				lastCommit = test.store->commit();
				wait(lastCommit);
				printf("ETA: %d nodes, %f seconds\n", i, (timer() - setupBegin) / i * (workload->nodeCount - i));
			}
		}
		std::cout << test.store->getStorageBytes().free << std::endl;
		...
		for (i = 0; i < workload->nodeCount; i++) {
			...
			if (result.present()) {
				ASSERT(result.get() == StringRef(test.makeKey(i)));
			} else {
				ASSERT(false);
			}
		}
...
ACTOR Future<Void> testKVStore(KVStoreTestWorkload* workload) {
	...
	if (workload->storeType == "ssd")
		...
		test.store = keyValueStorePmem(fn, id, 500e6); // 500MB
	else if (workload->storeType == "memory-1")
