...
void initHelp() {
	...
	helpMap["configure"] = CommandHelp(
		"configure [new] <single|double|triple|three_data_hall|three_datacenter|ssd|memory|pmem|proxies=<PROXIES>|logs=<LOGS>|resolvers=<RESOLVERS>>*",
...
void configure_generator(const char* text, const char *line, std::vector<std::string>& lc) {
	const char* opts[] = {"new", "single", "double", "triple", "three_data_hall", "three_datacenter", "ssd", "ssd-1", "ssd-2", "memory", "memory-1", "memory-2", "pmem", "proxies=", "logs=", "resolvers=", NULL};
	array_generator(text, line, opts, lc);
}
